...
class Phonebook
{
  ...
  static void Main()
  {
    ...
    Console.WriteLine("Would you like to add a person to your phone book? [Y/N]");
    string answer = Console.ReadLine();
    if (answer == "Y" || answer == "y") {
      AddContact();
    } else {
      Console.WriteLine("Would you like to lookup a number in your phone book? [Y/N]");
      string lookUpAnswer = Console.ReadLine();
      if (lookUpAnswer == "Y" || lookUpAnswer == "y") {
        LookUpContact();
      } else {
        Console.WriteLine("Are you finished with this program? [Y/N]");
